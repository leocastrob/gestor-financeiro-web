...
<template>
  <AppShell>
    ...
    <div v-else class="space-y-4 lg:space-y-0 lg:grid lg:grid-cols-2 lg:gap-6 lg:items-start">

      <!-- Coluna esquerda: resumo do mês + categorias -->
      <div class="space-y-4">
        ...
        <div v-if="gastosStore.transacoesVisiveis.length > 0"
          ...
          <!-- Saldo do mês — em destaque, largura cheia (é o número mais importante e o que mais
               sofria com 3 colunas apertadas cortando o valor com "…" em vez de mostrar o valor real) -->
          <div class="rounded-2xl p-4 flex items-center justify-between gap-3 border"
            ...
            <div class="min-w-0">
              ...
              <p class="text-2xl sm:text-3xl font-black font-mono tabular-nums"
                :class="gastosStore.saldoNumerico >= 0
            ...
            <p v-if="variacaoPercentual !== null" class="text-xs font-semibold text-right flex-shrink-0"
              ...
              {{ Math.abs(variacaoPercentual).toFixed(0) }}%<br>vs. {{ MESES[(gastosStore.filtroMes - 2 + 12) % 12] }}
            </p>
          </div>

          <!-- Despesas / Receitas lado a lado — metade da largura cada, bem mais espaço que um
               terço, então o valor cabe inteiro em qualquer tamanho de tela -->
          <div class="grid grid-cols-2 gap-2">
            <div class="bg-red-500/10 border border-red-500/20 rounded-2xl p-3 sm:p-4 flex flex-col gap-1 min-w-0">
              <p class="text-red-700/80 dark:text-red-400/70 text-xs font-semibold uppercase tracking-wider leading-tight">Despesas</p>
              <p class="text-lg sm:text-xl font-black text-red-600 dark:text-red-400 font-mono tabular-nums">
                {{ gastosStore.totalGastos }}
              </p>
            </div>

            <div class="bg-sky-500/10 border border-sky-500/20 rounded-2xl p-3 sm:p-4 flex flex-col gap-1 min-w-0">
              <p class="text-sky-700/80 dark:text-sky-400/70 text-xs font-semibold uppercase tracking-wider leading-tight">Receitas</p>
              <p class="text-lg sm:text-xl font-black text-sky-600 dark:text-sky-400 font-mono tabular-nums">
      ...
      <!-- Coluna direita: histórico + metas -->
      <div class="space-y-4">
