...
// Pega o telefone da URL e busca os gastos
const carregarGastos = () => {
  const telefone = route.params.telefone as string
  if (telefone) {
    gastosStore.buscarGastos(telefone)
  }
}

onMounted(carregarGastos)

// Recarrega se o parâmetro mudar na URL
watch(() => route.params.telefone, carregarGastos)
</script>
    ...
    <div class="max-w-2xl mx-auto bg-white rounded-2xl shadow-lg p-6 border border-slate-200">
      ...
      <div v-if="gastosStore.carregando" class="flex justify-center items-center py-10">
        <div class="animate-pulse flex flex-col items-center">
          ...
          <p class="text-slate-500 font-medium">Buscando dados...</p>
        </div>
      </div>
      ...
      <div v-else class="space-y-4">
        ...
        <div v-if="gastosStore.transacoes.length > 0"
          class="flex justify-between items-center p-4 bg-emerald-50 rounded-xl border border-emerald-200">
          <span class="text-lg font-bold text-emerald-800">Total de Gastos</span>
